...
# Disable uvicorn access logs (POST /rollout HTTP/1.1 200 OK spam)
import logging

# Patch uvicorn.Config to disable access logs by default
try:
    import uvicorn
    original_config_init = uvicorn.Config.__init__
    def patched_config_init(self, *args, **kwargs):
        kwargs.setdefault("access_log", False)  # Disable access logs
        return original_config_init(self, *args, **kwargs)
    uvicorn.Config.__init__ = patched_config_init
except Exception:
    pass  # If uvicorn not imported yet, will be patched when it loads

# Also disable logger directly
...
# Verbose logging control - set TASK_APP_VERBOSE=1 to enable debug logs
def _is_verbose() -> bool:
    """Check if verbose logging is enabled (checked dynamically each call)."""
    return False  # DISABLED - set to: os.getenv("TASK_APP_VERBOSE", "").lower() in {"1", "true", "yes"}

def _log(msg: str) -> None:
    """Print log message only if TASK_APP_VERBOSE is enabled."""
    if _is_verbose():
        print(msg, flush=True)

# Log environment at module load time for debugging (always show this one)
if _is_verbose():
    print(
        ...
        if split not in self._cache:
            try:
                ...
                try:
                    ...
                    _log(f"[Banking77Dataset] Cache load failed, trying download: {cache_exc}")
                    ds = _load_dataset(
                        dataset_name,
                        split=split,
                        trust_remote_code=False,
                ...
                _log(f"[Banking77Dataset] Successfully loaded {len(ds)} examples from '{dataset_name}' split '{split}'")
            except Exception as exc:
                # Preserve original exception details for debugging
                import traceback
                error_details = traceback.format_exc()
                _log(f"[Banking77Dataset] Dataset load failed: {exc}\n{error_details}")
                raise RuntimeError(
...
async def call_chat_completion(
    ...
    if inference_url_raw:
        ...
        if api_base_raw or base_url_raw:
            ...
            with contextlib.suppress(Exception):
                _log(f"[TASK_APP] ⚠️  inference_url is set ({route_base}), ignoring api_base/base_url")
    else:
    ...
    if "max_completion_tokens" in policy_config:
        ...
        with contextlib.suppress(Exception):
            _log(f"[TASK_APP] Using explicit max_completion_tokens: {max_tokens}")
    elif "max_tokens" in policy_config:
        # Legacy support for max_tokens
        max_tokens = policy_config.get("max_tokens")
        with contextlib.suppress(Exception):
            _log(f"[TASK_APP] Using legacy max_tokens: {max_tokens}")
    else:
        max_tokens = get_default_max_completion_tokens(model)
        with contextlib.suppress(Exception):
            _log(f"[TASK_APP] Using model-based default max_completion_tokens for {model}: {max_tokens}")

    # Loud route log
    with contextlib.suppress(Exception):
        _log(f"[TASK_APP] POLICY ROUTE → {inference_url}")
    ...
    _log(f"[TASK_APP] MESSAGES: {preview}")
    ...
    if api_key:
        ...
        if is_provider_host:
            ...
            with contextlib.suppress(Exception):
                _log(f"[TASK_APP] 🔐 DIRECT PROVIDER CALL with API key: {api_key[:12]}...{api_key[-4:]} (len={len(api_key)})")
                _log("[TASK_APP] 🔐 Using Authorization: Bearer header")
        else:
            headers["X-API-Key"] = api_key
            with contextlib.suppress(Exception):
                _log(f"[TASK_APP] 🔐 PROXY ROUTING with API key: {api_key[:12]}...{api_key[-4:]} (len={len(api_key)})")
                _log(f"[TASK_APP] 🔐 Headers being sent to proxy: {list(headers.keys())}")
    else:
        with contextlib.suppress(Exception):
            _log("[TASK_APP] ⚠️  NO API KEY PROVIDED!")
            _log("[TASK_APP] ⚠️  This will likely fail auth")
    ...
    _log(f"[TASK_APP] OUTBOUND: model={model} temp={temperature} max={max_tokens} tools=1 choice={TOOL_NAME}")
    ...
    _log(f"[TASK_APP] PROXY_TARGET: scheme={parsed.scheme} host={host} port={port} path={parsed.path}")
    ...
    if skip_dns_preresolution:
        _log(f"[TASK_APP] PROXY_DNS: Skipping DNS pre-resolution for direct provider host: {host}")
    else:
        try:
            ...
            _log(f"[TASK_APP] PROXY_DNS: resolved {host} -> {resolved_ip} (from {ips})")
            ...
            if resolved_ip and parsed.scheme == "https":
                ...
                headers["Host"] = host
                _log(f"[TASK_APP] PROXY_URL_REWRITTEN: {inference_url} (will use SNI with host={host}, Host header set)")
        except Exception as e:
            _log(f"[TASK_APP] PROXY_DNS_ERROR: {e}, continuing with original URL")
    ...
    with contextlib.suppress(Exception):
        ...
        _log(f"[TASK_APP] 📤 Sending POST to: {inference_url}")
        _log(f"[TASK_APP] 📤 With headers: {headers_log}")
        _log(f"[TASK_APP] 📤 Payload keys: {list(payload.keys())}")
        # Final assertion before sending
        if "X-API-Key" in headers:
            _log(f"[TASK_APP] ✅ X-API-Key IS in headers (len={len(headers['X-API-Key'])})")
        else:
            _log("[TASK_APP] ❌ X-API-Key NOT in headers!")
    ...
    try:
        ...
        if is_aiohttp:
            ...
            # Retry logic for connection errors (connection reset, server overload)
            max_retries = 3
            retry_delay = 0.5  # Start with 0.5s, double each retry
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    async with http_client.post(
                        ...
                        _log(f"[TASK_APP] RESPONSE_STATUS: {status_code}")
                        _log(f"[TASK_APP] RESPONSE_HEADERS: {dict(response.headers)}")

                        # Retry on 502/503/504 gateway errors
                        if status_code in (502, 503, 504):
                            if attempt < max_retries - 1:
                                _log(f"[TASK_APP] Gateway error {status_code}, retrying ({attempt + 1}/{max_retries})...")
                                await asyncio.sleep(retry_delay * (2 ** attempt))
                                continue
                            else:
                                # Last retry exhausted - raise error
                                _log(f"[TASK_APP] Gateway error {status_code} after {max_retries} retries, giving up")
                                raise HTTPException(status_code=status_code, detail=f"Gateway error {status_code} after {max_retries} retries")
                        ...
                        if status_code != 200:
                            try:
                                ...
                                _log(f"[TASK_APP] ❌ Error response from interceptor: {error_msg}")
                                _log(f"[TASK_APP] ❌ Full error JSON: {error_json}")
                                raise HTTPException(
                                ...
                                _log(f"[TASK_APP] ❌ Non-JSON error response: {error_text}")
                                raise HTTPException(
                                    status_code=status_code,
                        ...
                        try:
                            ...
                            _log(f"[TASK_APP] RESPONSE_JSON ({len(raw)} bytes): {raw}")
                        except Exception:
                            response_text = await response.text()
                            _log(f"[TASK_APP] RESPONSE_TEXT ({len(response_text)} bytes): {response_text}")
                            if status_code >= 400:
                ...
                except aiohttp.ClientOSError as conn_err:
                    # Connection reset, broken pipe, etc. - retry
                    last_conn_error = conn_err
                    if attempt < max_retries - 1:
                        wait_time = retry_delay * (2 ** attempt)
                        _log(f"[TASK_APP] Connection error ({type(conn_err).__name__}), retrying in {wait_time:.1f}s ({attempt + 1}/{max_retries})...")
                        await asyncio.sleep(wait_time)
                        continue
                    else:
                        _log(f"[TASK_APP] Connection error after {max_retries} retries: {conn_err}")
                        raise HTTPException(status_code=502, detail=f"Connection failed after {max_retries} retries: {conn_err}")
                except aiohttp.ServerDisconnectedError as disc_err:
                    # Server disconnected - retry
                    last_conn_error = disc_err
                    if attempt < max_retries - 1:
                        wait_time = retry_delay * (2 ** attempt)
                        _log(f"[TASK_APP] Server disconnected, retrying in {wait_time:.1f}s ({attempt + 1}/{max_retries})...")
                        await asyncio.sleep(wait_time)
                        continue
                    else:
                        _log(f"[TASK_APP] Server disconnected after {max_retries} retries")
                        raise HTTPException(status_code=502, detail=f"Server disconnected after {max_retries} retries")
        else:
        ...
        _log(f"[TASK_APP] POST_EXCEPTION: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
    ...
    with contextlib.suppress(Exception):
        ...
        _log(f"[TASK_APP] RESPONSE: usage={usage} choices={len(response_json.get('choices', []))} first_len={len(txt)} tool_calls={len(tc)}")
...
async def rollout_executor(request: RolloutRequest, fastapi_request: Request) -> RolloutResponse:
    ...
    _log(f"[TASK_APP] UPSTREAM_RESPONSE_JSON ({len(raw_upstream)} bytes): {raw_upstream}")
    ...
    _log(f"[TASK_APP] RAW_TOOL_CALLS: {tool_calls}")
        ...
        for tc in tool_calls:
            if tc.get("function", {}).get("name") == TOOL_NAME:
                ...
                try:
                    ...
                    # Handle case where LLM returns intent as list instead of string
                    if isinstance(intent_raw, list):
                        predicted_intent = " ".join(str(x) for x in intent_raw).strip()
                        _log(f"[TASK_APP] PARSED_TOOL_INTENT (from list): {predicted_intent}")
                    elif isinstance(intent_raw, str):
                        predicted_intent = intent_raw.strip()
                        _log(f"[TASK_APP] PARSED_TOOL_INTENT: {predicted_intent}")
                    else:
                        predicted_intent = str(intent_raw).strip() if intent_raw else ""
                        _log(f"[TASK_APP] PARSED_TOOL_INTENT (converted): {predicted_intent}")
                except Exception as e:
                    _log(f"[TASK_APP] TOOL_PARSE_ERROR: {args_str}, error: {e}")
    elif response_text:
        predicted_intent = response_text.strip().split()[0] if response_text.strip() else ""
        _log(f"[TASK_APP] CONTENT_FALLBACK_INTENT: {predicted_intent} text_len={len(response_text or '')}")

    # Gracefully handle missing predictions - return incorrect with score=0
    # instead of crashing (LLM sometimes returns malformed responses)
    if not str(predicted_intent or "").strip():
        _log(f"[TASK_APP] ⚠️  No prediction produced from proxy response - returning incorrect")
        predicted_intent = "__NO_PREDICTION__"  # Will never match expected intent

    # Ensure predicted_intent is a string (defensive check)
    predicted_intent = str(predicted_intent).strip() if predicted_intent else "__NO_PREDICTION__"

    expected_intent = sample["label"]
    is_correct = (predicted_intent.lower().replace("_", " ") == expected_intent.lower().replace("_", " "))
    reward = 1.0 if is_correct else 0.0

    _log(f"[TASK_APP] PREDICTION: expected={expected_intent} predicted={predicted_intent} correct={is_correct}")
    ...
    # CRITICAL: Extract inference_url from policy config with assertions
    policy_config = request.policy.config or {}
    inference_url = policy_config.get("inference_url")
    
    # ASSERTION: inference_url must be present in policy config for trace hydration
    if not inference_url:
        policy_keys = list(policy_config.keys())
        error_msg = (
            f"Missing inference_url in policy.config. "
            f"Keys present: {policy_keys}. "
            f"This is required for trace hydration and correlation ID extraction. "
            f"Baseline validation should set this via build_policy_config()."
        )
        _log(f"[TASK_APP] ❌ ERROR: {error_msg}")
        raise ValueError(error_msg)
    
    # ASSERTION: inference_url must be a non-empty string
    if not isinstance(inference_url, str) or not inference_url.strip():
        error_msg = (
            f"Invalid inference_url type/value: {type(inference_url)!r}={inference_url!r}. "
            f"Expected non-empty string."
        )
        _log(f"[TASK_APP] ❌ ERROR: {error_msg}")
        raise ValueError(error_msg)
    
    # ASSERTION: inference_url should contain correlation ID (cid=) for trace hydration
    if "cid=" not in inference_url:
        _log(f"[TASK_APP] ⚠️  WARNING: inference_url missing cid= parameter: {inference_url[:100]}...")
        # Don't fail here - some task apps might return v3 trace directly
        # But log it for debugging
    
    _log(f"[TASK_APP] ✅ inference_url extracted: {inference_url[:100]}{'...' if len(inference_url) > 100 else ''}")
    ...
    trajectory = RolloutTrajectory(
        ...
        length=1,
        inference_url=inference_url,  # Use directly, don't convert None to ""
    )
...
def build_config() -> TaskAppConfig:
    ...
    print("[banking77_task_app] Preloading dataset splits...")
    try:
        dataset.ensure_ready(AVAILABLE_SPLITS)
        print(f"[banking77_task_app] Dataset preloaded successfully: {[dataset.size(s) for s in AVAILABLE_SPLITS]} examples")
    except Exception as exc:
        print(f"[banking77_task_app] WARNING: Dataset preload failed: {exc}")
    ...
    async def startup_http_client(app: Any) -> None:
        try:
            import aiohttp
            # CRITICAL: LLM calls can take 5+ minutes, especially through interceptor
            # Set total timeout to 10 minutes (600s) to handle slow providers/interceptor
            # connect timeout: 30s (connection establishment)
            # sock_read timeout: 10min (reading response body - LLMs can be slow)
            timeout = aiohttp.ClientTimeout(
                total=600.0,  # 10 minutes total timeout
                connect=30.0,  # 30 seconds to establish connection
                sock_read=600.0,  # 10 minutes to read response (LLMs can be slow)
            )
            ...
            connector = aiohttp.TCPConnector(
                limit=100,  # Allow up to 100 concurrent connections (was 10)
                limit_per_host=100,  # Allow up to 100 per host (was 5)
            ...
            print("[banking77_task_app] Created app-level aiohttp client session singleton")
        except ImportError:
            # Fallback to httpx if aiohttp not available
            try:
                import httpx
                # Match aiohttp timeout: 10 minutes for LLM calls
                app.state.http_client = httpx.AsyncClient(
                    timeout=httpx.Timeout(600.0, connect=30.0, read=600.0),
                    limits=httpx.Limits(max_keepalive_connections=50, max_connections=100),
                )
                print("[banking77_task_app] Created app-level httpx client singleton (fallback)")
            except Exception as exc:
                print(f"[banking77_task_app] WARNING: Failed to create http client: {exc}")
                app.state.http_client = None
        except Exception as exc:
            print(f"[banking77_task_app] WARNING: Failed to create aiohttp client: {exc}")
            app.state.http_client = None
    ...
    async def shutdown_http_client(app: Any) -> None:
        ...
        if http_client is not None:
            try:
                ...
                print("[banking77_task_app] Closed app-level http client")
            except Exception as exc:
                print(f"[banking77_task_app] WARNING: Error closing http client: {exc}")
...
def fastapi_app():
    ...
    for route in routes_to_remove:
        ...
        print(f"[banking77] Removed default route: {getattr(route, 'path', 'unknown')}")
