...
Self-contained - no external adapter dependencies.
"""
...
# ============================================================================
# Banking77 Dataset Utilities (self-contained)
# ============================================================================

def load_banking77_dataset(split: str = "train") -> List[Dict[str, Any]]:
    """Load Banking77 dataset from HuggingFace."""
    # Use legacy_datasets for older-style datasets
    try:
        dataset = datasets.load_dataset("PolyAI/banking77", split=split)
    except RuntimeError:
        # Fallback: try without any special flags
        dataset = datasets.load_dataset("legacy-datasets/banking77", split=split)
    
    label_features = dataset.features["label"]
    
    examples = []
    for i, item in enumerate(dataset):
        intent = label_features.int2str(item["label"])
        examples.append({
            "index": i,
            "query": item["text"],
            "text": item["text"],
...
def get_available_intents() -> List[str]:
    """Get list of all 77 banking intents."""
    try:
        dataset = datasets.load_dataset("PolyAI/banking77", split="train")
    except RuntimeError:
        dataset = datasets.load_dataset("legacy-datasets/banking77", split="train")
    
    label_features = dataset.features["label"]
    return [label_features.int2str(i) for i in range(77)]


def _warn_if_dotenv_is_messy():
    """Check for common .env issues."""
    if not os.getenv("GROQ_API_KEY"):
        print("⚠️  Warning: GROQ_API_KEY not set. Make sure .env is loaded.")


# ============================================================================
# Learning Curve Tracker (minimal stub)
# ============================================================================
...
class LearningCurve:
    ...
    def record(self, rollout_count: int, performance: float, checkpoint_pct: float):
        self.points.append(LearningCurvePoint(rollout_count, performance, checkpoint_pct))


class LearningCurveTracker:
    """Minimal learning curve tracker."""
    ...
    def save(self, output_dir: Path):
        ...
        output_file = output_dir / f"{self.framework}_{self.benchmark}_learning_curve.json"
        data = {
            "framework": self.framework,
            "benchmark": self.benchmark,
            "total_budget": self.total_budget,
            "points": [
                {"rollout_count": p.rollout_count, "performance": p.performance, "checkpoint_pct": p.checkpoint_pct}
    ...
    with GEPALogger(str(gepa_log_file)):
        ...
        class BudgetTrackingAdapter:
            ...
            def evaluate(self, inputs, candidate, **kwargs):
                ...
                result = self._adapter.evaluate(inputs, candidate, **kwargs)
                
                # Track best score from results
                if result is not None:
                    ...
                    if score > self._best_score:
                        ...
                        self._candidates_scored += 1
                
                # Print progress every 10 calls or every 5 seconds
                now = time.time()
                if self.metric_call_count % 10 == 0 or (now - self._last_progress_print) > 5:
                    elapsed = now - self._start_time
                    pct = 100.0 * self.metric_call_count / self.max_metric_calls
                    rate = self.metric_call_count / elapsed if elapsed > 0 else 0
                    eta = (self.max_metric_calls - self.metric_call_count) / rate if rate > 0 else 0
                    print(
                        f"\r📊 Progress: {self.metric_call_count}/{self.max_metric_calls} ({pct:.0f}%) "
                        f"| Best: {self._best_score:.1%} | {elapsed:.0f}s elapsed, ~{eta:.0f}s remaining",
                        end="", flush=True
                    )
